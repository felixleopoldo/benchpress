...
simulationBiDAG(1, n, DAGs, bindata2n)
simulationBiDAGTabu(1, n, DAGs, bindata2n)


cl <- makeCluster(n_datasets)
bidag_sim_list_2n <- clusterApply(cl, c(1:n_datasets), simulationBiDAG, n = n, DAGs, bindata2n)
bidag_tabu_sim_list_2n <- clusterApply(cl, c(1:n_datasets), simulationBiDAGTabu, n = n, DAGs, bindata2n)

bidag_sim_list_10n <- clusterApply(cl, c(1:n_datasets), simulationBiDAG, n = n, DAGs, bindata10n)
bidag_tabu_sim_list_10n <- clusterApply(cl, c(1:n_datasets), simulationBiDAGTabu, n = n, DAGs, bindata10n)
stopCluster(cl)

saveRDS(bidag_sim_list_2n, "bidag_sim_list_2n.Rda")
saveRDS(bidag_sim_list_10n, "bidag_sim_list_10n.Rda")
saveRDS(bidag_tabu_sim_list_2n, "bidag_tabu_sim_list_2n.Rda")
saveRDS(bidag_tabu_sim_list_10n, "bidag_tabu_sim_list_10n.Rda")
...
# Try to add parfor since then all methds could be in the same for loop.
...
ss <- 2
data <- subset(sumROCdf[order(sumROCdf$threshold), ], ss == ss)
title <- paste("n=x, sample size ", str(ss), "n", sep = "")
plot_ROC(data)
dev.off()
