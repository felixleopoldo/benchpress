...
* ``alpha`` The BIC penalty parameter (lambda) for the Gaussian BIC score. 
* ``baseline`` Base search algorithm (ges, xges, ops, lges-safe, or lges-cons). Note: "boss" is not supported here -- it requires an initial graph produced beforehand by BOSS (part of the Tetrad library), a separate, external step this module does not cover. 
* ``delete_op`` Passed straight through to Search's --delete_op flag. 
* ``variant`` Perturbation strategy on top of the baseline, 0 through 3, where 2 is the paper's proposed component-wise parent-deletion (DP) operator (Algorithm 2) and 3 additionally schedules single-node and single-edge phases after it. 0: no perturbation (vanilla baseline). 1: single-edge deletion (XGES-style). 2: component-wise parent deletion (DP). 3: three-phase scheduled perturbation (DP -> single-node -> single-edge). 
