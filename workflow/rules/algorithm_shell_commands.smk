...
def alg_shell(algorithm):
    if algorithm == "gt13_multipair":
        ...
        return  """
                if [ {wildcards.timeout} = \"None\" ]; then
                    if [ {wildcards.datatype} = \"discrete\" ]; then
                        /usr/bin/time -f \"%e\" -o {output.time} parallelDG_loglinear_sample -M {wildcards.M} -R {wildcards.R} -f {input} -o . -F {output.adjvecs} --pseudo_observations {wildcards.pseudo_obs} -s {wildcards.mcmc_seed};
                    elif [ {wildcards.datatype} = \"continuous\" ]; then
                     ...
                     if [ {wildcards.datatype} = \"discrete\" ]; then
                        /usr/bin/time -f \"%e\" -o {output.time} timeout -s SIGKILL {wildcards.timeout} bash -c  'parallelDG_loglinear_sample -M {wildcards.M} -R {wildcards.R} -f {input} -o . -F {output.adjvecs} --pseudo_observations {wildcards.pseudo_obs} -s {wildcards.mcmc_seed}';
                    elif [ {wildcards.datatype} = \"continuous\" ]; then
