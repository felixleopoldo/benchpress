...
#         filename=graph_plots_feature_pattern(feature="csvs")
#     output:
#         filename=graph_plots_feature_pattern(feature="cpdags")
#     container:
...
rule dag_to_cpdag:
    input:
        filename="{whatever}/evaluation/csvs/{something}/adjmat.csv"
    output:
        filename="{whatever}/evaluation/cpdag_csvs/{something}/adjmat.csv"
    container:
        "docker://bpimages/pcalg:2.7-8"
    params:
        to="cpdag"
...
rule adjmat_to_dot:
    ...
    output:
        filename="{output_dir}/evaluation/graph_plots/dot/{something}/graph.dot" # maybe need a temp dot folder
    container:
...
rule adjmat_to_dot:
    input:
        "workflow/scripts/utils/adjmat_to_dot.py",
        filename="{output_dir}/evaluation/graph_plots/cpdag_csvs/{something}/adjmat.csv"  # true graph has adjmat in the path and estimated does not.
    output:
        filename="{output_dir}/evaluation/graph_plots/dot/{something}/graph.dot" # maybe need a temp dot folder
    container:
        docker_image("trilearn")
...
# TODO: CPDAG stuff is not working yet
# Maybe need a specific for cpdags
rule dot_to_plot:
    input:
        "{output_dir}/evaluation/graph_plots/dot/type=cpdag/{something}/graph.dot"
        #"{output_dir}/evaluation/graph_plots/dot/type=cpdag/{something}/graph.dot"?
        
    output:
        "{output_dir}/evaluation/graph_plots/{feature}/{something}/graph.png"
    container:
        docker_image("trilearn")
    wildcard_constraints:
        feature="(graphs|cpdags)"
    shell:
        """
        if [ -s {input} ]; then
...
rule graph_plots:
    input:
        ...
        cpdags=[d for d in graph_plots_conf_to_feature_files(filename="graph", 
                                                        ext="png", 
                                                        eval_module="graph_plots", 
                                                        module_feature="cpdags", 
                                                        feature_argstring="") if config["benchmark_setup"]["evaluation"]["graph_plots"]["cpdags"]],
