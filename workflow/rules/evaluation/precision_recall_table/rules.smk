# This module computes, for each (sim_setup, alg_conf) cell, the mean TP/P
# (recall) and FP/P averaged over seeds — tabulated as:
# rows = sim_setups, columns = alg_conf IDs x {TP/P, FP/P}.
# One table per graph_type.


def _prt_collect_adjmats_over_seeds_single_param(sim_setup, alg_string, graph_type="original"):
...
for bmark_setup in config["benchmark_setup"]:
    if "precision_recall_table" not in bmark_setup["evaluation"]:
        continue
    eval_conf = bmark_setup["evaluation"]["precision_recall_table"]
    ...
    graph_types_raw = eval_conf.get("graph_type", "original")
    ...
    if row_labels_conf is not None and len(row_labels_conf) != len(bmark_setup["data"]):
        raise ValueError(
            f"precision_recall_table '{bmark_setup_title}': row_labels has "
    ...
    if latex_row_labels_conf is not None and len(latex_row_labels_conf) != len(bmark_setup["data"]):
        raise ValueError(
            f"precision_recall_table '{bmark_setup_title}': latex_row_labels has "
    ...
    for graph_type in graph_types:
        output_dir = (f"results/output/{bmark_setup_title}/"
                      f"precision_recall_table/graph_type={graph_type}")
                    ...
                    for alg_conf in alg_conf_list:
                        if alg_conf["id"] == alg_conf_id:
                            for alg_str in json_string[alg_conf_id]:
                                cell_files += _prt_collect_adjmats_over_seeds_single_param(
        ...
        rule:
            name:
                f"precision_recall_table_{bmark_setup_title}_{graph_type}"
            input:
