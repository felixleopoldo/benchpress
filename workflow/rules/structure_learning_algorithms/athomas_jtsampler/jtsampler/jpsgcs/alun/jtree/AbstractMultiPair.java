...
abstract public class AbstractMultiPair<V> extends JTreeSampler<V>
{
	...
	public UpdateResult randomConnection()
	{
		...
		Separator<V> S = jt.separators.sample();
		...
		if (X.isEmpty())
			//return 2;
			return new UpdateResult(2);

		if (Y.isEmpty())
			//return 3;
			return new UpdateResult(3);
...
/* Handle sink for discrete variables.*/
		...
		if (random().nextDouble() > Aij)
			//return 8;
			return new UpdateResult(8);

		double delta = deltaScore(C,X,Y,type);
		if (Math.log(random().nextDouble()) * getTemperature() > delta)
			//return 9;
			return new UpdateResult(9);
		...
		int ret = proposeDisconnections(X,Y,Cx,Cy,C);
		if (ret > 0)
			return new UpdateResult(ret);
