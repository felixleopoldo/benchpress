...
// Private data.
	...
	public SubMatrix subCovariance(Set<Integer> a)
	{
		...
		//double n = delta + data.nRows() + q.length - 1;
		double n = delta + data.nRows() - 2;
		...
		return new SubMatrix(q,s);
	}

	public SubMatrix subPrecision(Set<Integer> a)
	{
		SubMatrix s = subCovariance(a);
		s.matrix = Matrix.inverse(s.matrix);

		double m = delta + data.nRows() + s.index.length - 1;
		double n = delta + data.nRows() - 2;
