...
CP=$(pwd)/workflow/rules/structure_learning_algorithms/dualGraphLearner/jtsampler

if [ ${snakemake_wildcards[timeout]} = "None" ]; then
    java -classpath $CP EstimateGM \
...
## Rscript $CP/../screening.R  ${snakemake_output[adjmat]} ${snakemake_output[seqgraph]} ${snakemake_output[time]} ${snakemake_input[data]} ${snakemake_output[ntests]}
