...
tcheckTriple <- function(a, b, c, nbrsA, nbrsC, sepsetA, sepsetC, suffStat,
    ...
    if ((nn <- length(nbrsA)) > 0) {
        ...
        for (i in 1:nrow(allComb)) {
            ...
            # Testabliliy:
            # If R_X is either labels[a] or labels[c], and X is in S
            # it is untestable so we skip the test. Rely in the once without it.
            #message("Does this work?")
            #message("a: ", labels[a])
            #message("c: ", labels[c])
            #message("S: ")
            #message(paste(labels[S], collapse = ", "))
            # get the X out of R_X

            if (grepl("^R_", labels[a])) {
                subst <- which(labels == gsub("^R_", "", labels[a]))
                #message("subst: ", subst)
                #message("subst name:", labels[subst])
                if (subst %in% S) {
                    print(paste("Untestable because ", labels[a], " has substantive variables ", labels[subst], " in the sepset", labels[S]))
                    # skip
            ...
            if (grepl("^R_", labels[c])){
                subst <- which(labels == gsub("^R_", "", labels[c]))
                #message("subst: ", subst)
                #message("subst name:", labels[subst])
                if (subst %in% S) {
                    #print(paste("Untestable because ", labels[c], " has substantive variables ", labels[subst], " in the sepset", labels[S]))
                    # skip
                    #stop("TESSST 2")
                    next
                }
    ...
    lapply(list(
        decision = res, version = version, SepsetA = sepsetA,
        SepsetC = sepsetC
    ), as.integer)
}
