...
if (file.info(snakemake@input[["csv"]])$size == 0) { 
     ...
     cat("Time-out",file=snakemake@output[["ntests"]], sep="\n")
     cat("Time-out",file=snakemake@output[["SHD_cpdag"]], sep="\n")
} else {
...
ggplot() + {
  ...
  facet_wrap(. ~ adjmat + bn + data , ncol = 2) +
  ggtitle("Elapsed time per seed") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) 
  ggsave(file = snakemake@output[["elapsed_time_fine"]])
